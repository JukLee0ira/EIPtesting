...
```json
{
  "config": {
    "chainId": 20986,
    "londonBlock": 0,  // 激活高度（示例）
    ...
  }
}
```

- **设置环境变量**：`RPC_URL` + 至少4个私钥（`PRIVATE_KEYS=key1,key2,key3,key4`，请参阅根目录下的`example.env`）。
...
**测试命令：**
```bash
npx hardhat test test/eip2935.test.ts --grep "E2. Test Access Control Security" --network <网络选项>
```
...
**测试命令：**
```bash
npx hardhat test test/eip2935.test.ts --grep "E3. Test State Consistency" --network <网络选项>
```
...
**测试命令：**
```bash
npx hardhat test test/eip2935.test.ts --grep "F1. Test EIP-2929 Interaction" --network <网络选项>
```

**测试目的：**
- 验证与EIP-2929 Gas定价规则的交互

**测试步骤：**
1. 激活后首次调用历史合约
2. 对相同存储槽的后续调用
3. 验证Gas定价遵循EIP-2929规则
...
**测试命令：**
```bash
npx hardhat test test/eip2935.test.ts --grep "F2. Test BLOCKHASH Independence" --network <网络选项>
```

**测试目的：**
- 验证EIP-2935不影响原生BLOCKHASH操作

**测试步骤：**
1. 对近期的区块调用BLOCKHASH操作码
2. 与历史合约查询结果比较
3. 验证BLOCKHASH仍独立工作
...
**功能特性：**
- 为最后8191个区块哈希提供环形缓冲区存储
- Get操作用于查询历史哈希
- Set操作用于存储父区块哈希（仅限系统调用）
- 通过调用者验证实现访问控制

**主要函数：**
```solidity
// Get操作 - 从EVM调用
function get(uint256 blockNumber) external view returns (bytes32 hash)

// Set操作 - 仅可由SYSTEM_ADDRESS调用
function set(bytes32 parentHash) external
```

**存储布局：**
- 环形缓冲区大小：8191个槽位
- 存储位置：(blockNumber - 1) % HISTORY_SERVE_WINDOW
- 有效查询范围：[block.number-HISTORY_SERVE_WINDOW, block.number-1]
...
function verifyStorageSlot(uint256 blockNumber, bytes32 expectedHash) external view
function generateBlockHash(uint256 blockNumber) external pure returns (bytes32)
```
